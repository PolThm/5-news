...
def test_one_angle_is_requested_per_published_item_and_zone() -> None:
    """Per pair, not per item: the same event needs a different angle in the
    France and Spain Briefings, and requesting one per item would give both the
    same text."""
    from pipeline.config import zone_by_slug
    from pipeline.stages.briefing_matrix import zone_angles
    from pipeline.stages.rank import ZoneRanking

    shared = _cluster("shared", sources=2, countries=["france", "spain"])
    france = zone_by_slug("france")
    spain = zone_by_slug("spain")
    rankings = [
        ZoneRanking(requested_zone=france, served_zone=france, ranked_clusters=[shared]),
        ZoneRanking(requested_zone=spain, served_zone=spain, ranked_clusters=[shared]),
    ]

    pairs = zone_angles(rankings)

    assert {slug for _, slug in pairs} == {"france", "spain"}
    assert len(pairs) == 2


def test_the_world_briefing_needs_no_angle() -> None:
    """Its items already carry the shared judgment, which is written for no
    territory in particular -- exactly what a World Briefing wants, and one
    fewer request per item."""
    from pipeline.config import zone_by_slug
    from pipeline.stages.briefing_matrix import zone_angles
    from pipeline.stages.rank import ZoneRanking
    ...
    rankings = [
        ZoneRanking(
            requested_zone=world,
            served_zone=world,
            ranked_clusters=[_cluster("a", sources=2, countries=["france", "spain"])],
        )
    ]

    assert zone_angles(rankings) == []


def test_a_fallen_back_briefing_gets_the_angle_it_is_actually_served() -> None:
    """FR-16: a Briefing that fell back shows its Continent's selection, so the
    judgment on the page must be the Continent's. Writing for the requested Zone
    would put the wrong territory's angle on the page."""
    from pipeline.config import zone_by_slug
    from pipeline.stages.briefing_matrix import zone_angles
    from pipeline.stages.rank import ZoneRanking
    ...
    rankings = [
        ZoneRanking(
            requested_zone=zone_by_slug("france"),
            served_zone=zone_by_slug("europe"),
            ranked_clusters=[_cluster("a", sources=2, countries=["france", "spain"])],
        )
    ]

    assert [slug for _, slug in zone_angles(rankings)] == ["europe"]
