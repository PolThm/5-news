...
describe("formatTimestamp", () => {
  ...
  it("renders the date day-first in every language", () => {
    // Bare "en" resolves to en-US and would render "Aug 20", month-first and
    // alone among the three. DATE_LOCALE pins it to en-GB.
    expect(formatTimestamp(iso, "en", "Europe/Paris")).toContain("20 Aug");
    expect(formatTimestamp(iso, "fr", "Europe/Paris")).toContain("20 août");
    expect(formatTimestamp(iso, "es", "Europe/Paris")).toContain("20 ago");
  });

  it("makes a stale Briefing distinguishable from a fresh one", () => {
    // The reason the date is there at all: on 2026-08-24 a stalled cycle left
    // four-day-old Briefings that a time-only header rendered as if current.
