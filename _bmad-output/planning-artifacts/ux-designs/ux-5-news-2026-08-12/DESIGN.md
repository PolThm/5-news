...
- **Source Serif 4** carries the mad-libs sentence and every item headline — the register of a printed dispatch. Set at `display-lg`/`display-lg-mobile` for the sentence itself (the page's single largest visual element, per FR-1/FR-2/FR-3's framing of the sentence as the primary control), `headline-sm` for individual item titles.
